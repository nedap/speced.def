...
(def-with-doc ::spec-metadata
   ...
   * argument positions, i.e. before a given argument in the argument vector.
     * within arguments, any destructured _symbols_ can also be attached metadata.
       * Note that only symbols are observed: attaching metadata to hashmaps/vectors will accomplish nothing.
